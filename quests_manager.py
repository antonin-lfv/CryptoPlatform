...
class Quests_manager:
    ...
    def recover_quest(user_id, step, quest_type):
        ...
        user_rewards = UserQuestRewards.query.filter_by(
            user_id=user_id, step=step, quest_type=quest_type
        ).first()
        # If None, create with reward_claimed field as True
        if user_rewards is None:
            user_rewards = UserQuestRewards(
                user_id=user_id, step=step, quest_type=quest_type, reward_claimed=True
            )
        ...
        # If not None, set reward_claimed as True if it is False else return an error
            ...
            if user_rewards.reward_claimed:
                return {
                    "status": "error",
                    "message": f"User {user_id} has already claimed the reward for "
        ...
        return {
            "status": "success",
            "message": f"User {user_id} has claimed "
