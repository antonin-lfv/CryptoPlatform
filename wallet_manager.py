...
from models import (
    ...
    CryptoWalletDailySnapshot,
    CryptoWalletEvolution,
    UserNFT,
    NFT,
    Position,
    User,
...
class wallet_manager:
    ...
    def get_user_balance(self, user):
        ...
        for wallet in wallets:
            # Get latest price
            balance = self.crypto_manager.get_USD_from_crypto(
                wallet.symbol, wallet.quantity
            )
            ...
            user_balance["crypto_balance_by_symbol"][wallet.symbol]["balance"] = round(
                user_balance["crypto_balance_by_symbol"][wallet.symbol]["balance"], 2
            )
        ...
        for nft in nfts:
            ...
            USD_price = CryptoDataManager().get_USD_from_crypto(
                "ETH-USD", NFT_item.price
            )
        ...
        user_balance["crypto_balance_format"] = "{:,}".format(
            user_balance["crypto_balance"]
        )
        user_balance["web3_balance_format"] = "{:,}".format(
            user_balance["web3_balance"]
        )
    ...
    def get_user_specific_balance(self, user, symbol):
        ...
        for wallet in wallets:
            if wallet.symbol == symbol:
                ...
                user_balance["USD"] += self.crypto_manager.get_USD_from_crypto(
                    wallet.symbol, wallet.quantity
                )
    ...
    def get_game_wallet(user):
        ...
        if user.game_wallet:  # check if user has a game wallet
            ...
            return {"mini_wallet": 0, "bank_wallet": 0}
    ...
    def get_crypto_wallet_evolution(user):
        ...
        crypto_wallet_evolution = (
            CryptoWalletEvolution.query.filter_by(user_id=user.id)
            .order_by(CryptoWalletEvolution.date.desc())
            .all()
        )
        ...
        for evolution in crypto_wallet_evolution:
            wallet_evolution.append(
                {"date": evolution.date, "value": evolution.quantity}
            )
    ...
    def update_crypto_wallet_evolution(self, user):
        ...
        # Get wallet evolution of user sorted by date
        crypto_wallet_evolution = CryptoWalletEvolution.query.filter_by(
            user_id=user.id
        ).all()
    ...
    def update_wallet_daily_snapshot(user, quantity):
        ...
        # Get latest snapshot
        latest_snapshot = (
            CryptoWalletDailySnapshot.query.filter_by(user_id=user.id)
            .order_by(CryptoWalletDailySnapshot.date.desc())
            .first()
        )
    ...
    def get_wallet_crypto_transactions_history(user):
        ...
        if not user.wallets:
            ...
            return {"wallet_history": wallet_history}
        for i in range(len(user.wallets)):
            history_wallet = CryptoTransactionHistory.query.filter_by(
                wallet_id=user.wallets[i].id
            ).all()
    ...
    def get_crypto_wallet_daily_snapshot(user):
        ...
        today = datetime.utcnow().date()
        wallet_daily_snapshot = []
        daily_snapshot_wallet = CryptoWalletDailySnapshot.query.filter_by(
            user_id=user.id
        ).all()
        ...
        for snapshot in daily_snapshot_wallet:
            wallet_daily_snapshot.append(
                {"date": snapshot.date, "value": snapshot.quantity}
            )
        ...
        for i in range(len(wallet_daily_snapshot)):
            if i > 0:
                wallet_daily_snapshot[i]["value"] += wallet_daily_snapshot[i - 1][
                    "value"
                ]
        ...
        for single_date in dates_to_get:
            ...
            if current_value is not None:
                filled_wallet_snapshot.append(
                    {"date": single_date, "value": current_value}
                )
    ...
    def buy_crypto_with_USD(
        self, user, symbol, From, quantity_crypto=None, quantity_USD=None
    ):
        """
        Buy crypto with USD
        ...
        if quantity_crypto:
            ...
            quantity_USD = self.crypto_manager.get_USD_from_crypto(
                symbol, quantity_crypto
            )
        ...
        # Update wallet
        if quantity_USD:
            wallet.quantity += self.crypto_manager.get_crypto_from_USD(
                symbol, quantity_USD
            )
        else:
        ...
        if quantity_USD:
            self.add_transaction_to_wallet_history(
                wallet,
                "buy",
        ...
        if quantity_USD:
            ...
            self.update_wallet_daily_snapshot(
                user, self.crypto_manager.get_USD_from_crypto(symbol, quantity_crypto)
            )
    ...
    def buy_crypto_with_crypto(
        ...
        """
        # Get user wallet
        wallet_crypto_to_sell = CryptoWallet.query.filter_by(
            user_id=user.id, symbol=symbol_to_sell
        ).first()
        ...
        if quantity_to_sell:
            ...
            quantity_to_buy = self.crypto_manager.get_crypto_from_crypto(
                symbol_to_sell, symbol_to_buy, quantity_to_sell
            )
        # If we have quantity_to_buy, convert it to quantity_to_sell
        else:
            quantity_to_buy = float(quantity_to_buy)
            quantity_to_sell = self.crypto_manager.get_crypto_from_crypto(
                symbol_to_buy, symbol_to_sell, quantity_to_buy
            )
        ...
        wallet_crypto_to_sell.quantity = max(
            wallet_crypto_to_sell.quantity - quantity_to_sell, 0
        )

        # Get user wallet for symbol_to_buy
        wallet_crypto_to_buy = CryptoWallet.query.filter_by(
            user_id=user.id, symbol=symbol_to_buy
        ).first()
        # If wallet does not exist, create it
        ...
        self.add_transaction_to_wallet_history(
            wallet_crypto_to_sell, "sell", quantity_to_sell
        )
        self.add_transaction_to_wallet_history(
            wallet_crypto_to_buy, "buy", quantity_to_buy
        )
    ...
    def buy_with_crypto(self, user, symbol, quantity):
        ...
        user_wallet = CryptoWallet.query.filter_by(
    ...
    def place_position(self, user_id, position_json):
        ...
        positions = Position.query.filter_by(
            user_id=user_id, symbol=position_json["symbol"], status="open"
        ).all()
        if len(positions) >= 5:
            return {
                "message": "You can't have more than 5 positions for the same symbol",
                "success": False,
            }
        ...
        for position in positions:
            if position.created_at.date() == today and position.status == "open":
                return {
                    "message": "You can't have more than 1 opened position for the same symbol today",
                    "success": False,
                }
        ...
        # if not, return an error
        user_wallet = CryptoWallet.query.filter_by(
            user_id=user_id, symbol=position_json["symbol"]
        ).first()
        if not user_wallet:
        ...
        new_position.current_usd_price = self.crypto_manager.get_USD_from_crypto(
            position_json["symbol"], position_json["price"]
        )
        new_position.usd_entry_price = new_position.current_usd_price
        new_position.token_entry_price = self.crypto_manager.get_USD_from_crypto(
            position_json["symbol"], 1
        )
    ...
    def close_position(self, user_id, position_id):
        ...
        if position:
            ...
            profit = (
                lambda_usd_to_crypto(position.usd_entry_price)
            ...
            stop_loss_value_usd = (
                ...
                else position.stop_loss_percentage * position.usd_entry_price / 100
            )
            take_profit_value_usd = (
                position.take_profit_value
                if position.take_profit_value
                else position.take_profit_percentage * position.usd_entry_price / 100
            )
            ...
            position.current_pourcentage_profit = round(
                profit / lambda_usd_to_crypto(position.usd_entry_price) * 100, 2
            )
    ...
    def get_opened_positions(user_id, symbol):
        ...
        positions = Position.query.filter_by(
            user_id=user_id, status="open", symbol=symbol
        ).all()
        positions_json = []
        for position in positions:
            positions_json.append(
                {
                    ...
                    "stop_loss_format": (
                        f"{position.stop_loss_value}$"
                        if position.stop_loss_value
                        else f"{position.stop_loss_percentage}%"
                    ),
                    "take_profit_format": (
                        f"{position.take_profit_value}$"
                        if position.take_profit_value
                        else f"{position.take_profit_percentage}%"
                    ),
                    ...
                    "usd_entry_price": round(
                        position.usd_entry_price, 3
                    ),  # price set by the user in USD
                    "token_entry_price": round(
                        position.token_entry_price, 3
    ...
    def get_last_closed_positions(user_id, symbol):
        ...
        positions = (
            Position.query.filter_by(user_id=user_id, status="closed", symbol=symbol)
    ...
    def update_positions(self, user_id):
        ...
        for position in positions:
            ...
            current_price = self.crypto_manager.get_USD_from_crypto(
            ...
            if user_prediction == "high":
                if current_price > position.usd_entry_price:
                    position.current_usd_profit = (
                        current_price - position.usd_entry_price
                    ) * position.leverage
                    position.current_pourcentage_profit = (
                        (current_price / position.usd_entry_price - 1)
                        * 100
                        * position.leverage
                    )
                else:
                    position.current_usd_profit = (
                        (position.usd_entry_price - current_price)
                        * position.leverage
                        * -1
                    )
                    position.current_pourcentage_profit = (
                        (1 - current_price / position.usd_entry_price)
                        * 100
                        * position.leverage
                        * -1
                    )
            ...
            position.current_pourcentage_profit = round(
                position.current_pourcentage_profit, 2
            )

            if (
                position.stop_loss_percentage is not None
                and position.current_pourcentage_profit
                <= -position.stop_loss_percentage
            ):
                position.status = "closed"
            elif (
                position.take_profit_percentage is not None
                and position.current_pourcentage_profit
                >= position.take_profit_percentage
            ):
                position.status = "closed"
            elif (
                position.stop_loss_value is not None
                and position.current_usd_profit <= -position.stop_loss_value
            ):
                position.status = "closed"
            elif (
                position.take_profit_value is not None
                and position.current_usd_profit >= position.take_profit_value
            ):
                position.status = "closed"
            ...
            if position.status == "closed":
                ...
                profit = (
                    self.crypto_manager.get_crypto_from_USD(
                        position.symbol, position.usd_entry_price
                    )
                    * (position.current_pourcentage_profit + 100)
                    / 100
                )
                stop_loss_value_usd = (
                    position.stop_loss_value
                ...
                take_profit_value_usd = (
                    position.take_profit_value
                    if position.take_profit_value
                    else position.take_profit_percentage
                    * position.usd_entry_price
                    / 100
                )
                stop_loss_value_crypto = self.crypto_manager.get_crypto_from_USD(
                    position.symbol, stop_loss_value_usd
                )
                take_profit_value_crypto = self.crypto_manager.get_crypto_from_USD(
                    position.symbol, take_profit_value_usd
                )
                if profit < 0:
                    profit = max(profit, -stop_loss_value_crypto)
                    to_pay = position.price - profit
                ...
                position.current_usd_profit = round(
                    self.crypto_manager.get_USD_from_crypto(position.symbol, profit), 2
                )
                position.current_pourcentage_profit = round(
                    profit
                    / self.crypto_manager.get_crypto_from_USD(
