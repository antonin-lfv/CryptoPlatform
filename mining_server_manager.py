...
class Mining_server_manager:
    ...
    def check_for_server_payment(user_id):
        ...
        closest_next_earning_date = (
            UserServer.query.filter_by(user_id=user_id)
            .order_by(UserServer.next_earning_date)
            .first()
            .next_earning_date
        )
        ...
        user_total_balance = round(
            CryptoDataManager().get_crypto_from_USD("BTC-USD", user_total_balance), 2
        )
        ...
        for server_instance in user_server_instances:
            ...
            if server_instance.next_earning_date <= today_date:
                ...
                number_of_days_since_last_earning = (
                    today_date - server_instance.next_earning_date
                ).days + 1
                # Compute the total earnings for the server instance
                total_earnings = (
                    server_details.power
                    * server_instance.instances_number
                    * number_of_days_since_last_earning
                    * BONUS_FROM_BTC_WALLET
                )
                # Convert the total earnings to USD
                total_earnings_USD = cached_convert_fct(
                    server_details.symbol + "-USD", total_earnings
                )
                # Update the user's wallet
                wallet_manager().receive_crypto(
                    user, server_details.symbol + "-USD", total_earnings
                )
    ...
    def restart_mining_servers_price(user_id):
        ...
        if user.role != "ADMIN":
            return {
                "status": "error",
                "message": "You are not authorized to restart the mining servers",
            }
        else:
            ...
            return {
                "status": "success",
                "message": "Mining servers restarted successfully",
            }
    ...
    def get_user_mining_servers_invoices(user, server_name):
        ...
        user_server_type_invoices = ServerInvoices.query.filter_by(
            server_id=server_type_id, user_id=user.id
        ).all()
        # sort by period
        user_server_type_invoices = sorted(
            user_server_type_invoices, key=lambda x: x.period, reverse=True
        )
        invoices_list = []
    ...
    def get_all_servers(user_id):
        ...
        for server_item in servers:
            ...
            dict_to_add = {
                ...
                "number_of_servers": user_server_instances_dict.get(
                    f"{server_item.id}_buy", 0
                ),
            }
    ...
    def get_user_mining_server_details(server_id, user_id):
        ...
        user_server_details = UserServer.query.filter_by(
        ...
        output = {
            "number_of_servers": 0,
            "total_buy_amount": 0,
            "power": round(server_details.power, 2),
            "symbol": server_details.symbol,
        ...
        output["power_USD"] = round(
            cached_convert_fct(server_details.symbol + "-USD", server_details.power), 2
        )

        for server in user_server_details:
            output["number_of_servers"] += server.instances_number
            output["total_buy_amount"] += (
                server.server.buy_amount * server.instances_number
            )
            output["total_buy_amount_USD"] += cached_convert_fct(
                server.server.symbol + "-USD",
                server.server.buy_amount * server.instances_number,
            )
    ...
    def buy_server(self, server_id, user_id, number_of_servers_to_buy):
        ...
        number_of_servers_bought = self.get_user_mining_server_details(
            server_id, user_id
        )["number_of_servers"]
        ...
        if number_of_servers_bought + number_of_servers_to_buy > max_servers:
            return {
                "success": False,
                "message": "You will exceed the maximum number of bought servers for this "
                "server type",
            }
        ...
        user_wallet = wallet_manager().get_user_specific_balance(
            user, server_details.symbol + "-USD"
        )
        # Get the user's server details
        user_server_details = UserServer.query.filter_by(
            user_id=user_id, server_id=server_id
        ...
        if (
            ...
            # If yes, update the user's wallet
            response = wallet_manager().buy_with_crypto(
                user,
            ...
            if user_quest_stats:
                ...
                user_quest_stats = UserQuestsStats(
                    user_id=user_id,
                    nfts_bought=0,
                    nfts_sold=0,
                    bids_made=0,
            ...
            return {
                "success": False,
                "message": "You do not have enough crypto to buy this server",
            }
    ...
    def sell_server(server_id, user_id, number_of_servers_to_sell):
        ...
        # Get the number of servers for this server type
        number_of_servers_bought = sum(
            server.instances_number for server in user_server_details
        )
        ...
        if number_of_servers_bought < number_of_servers_to_sell:
            return {
                "success": False,
                "message": "You do not have enough servers to sell",
            }
        else:
    ...
    def get_total_servers(user_id):
        ...
        n_number_of_servers_bought = sum(
            server.instances_number for server in user_server_details
        )
    ...
    def get_total_power(user_id):
        ...
        for server in user_server_details:
            ...
            power_in_usd = (
                conversion_rates[server.server.symbol + "-USD"] * server.server.power
            )
...
def find_invoice(invoices_list, elem):
    for invoice in invoices_list:
        if all(
            invoice[key] == elem[key]
            for key in elem
            if key not in ["number_of_instances", "amount"]
        ):
            return invoice
    return None
