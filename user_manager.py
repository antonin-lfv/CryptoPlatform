...
class UserManager:
    ...
    def switch_notifications_active(user_id):
        try:
            ...
            return {
                ...
                "active": user.notifications_active,
            }
        except Exception as e:
    ...
    def get_user_in_leaderboard(user_id):
        ...
        for user in users:
            ...
            user_dict["total_balance"] = (
                balance["crypto_balance"] + balance["web3_balance"]
            )
            # Convert all balances to BTC
            user_dict["crypto_balance"] = round(
                CryptoDataManager().get_crypto_from_USD(
                    "BTC-USD", user_dict["crypto_balance"]
                ),
                2,
            )
            user_dict["web3_balance"] = round(
                CryptoDataManager().get_crypto_from_USD(
                    "BTC-USD", user_dict["web3_balance"]
                ),
                2,
            )
            user_dict["total_balance"] = round(
                CryptoDataManager().get_crypto_from_USD(
                    "BTC-USD", user_dict["total_balance"]
                ),
                2,
            )
            # Format the balances with comma between thousands
            user_dict["crypto_balance_format"] = "{:,}".format(
                user_dict["crypto_balance"]
            )
            user_dict["web3_balance_format"] = "{:,}".format(user_dict["web3_balance"])
            user_dict["total_balance_format"] = "{:,}".format(
                user_dict["total_balance"]
            )
