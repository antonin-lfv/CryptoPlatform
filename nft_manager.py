...
class NFT_manager:
    ...
    def update_NFT_price():
        ...
        last_days = (
            CryptoPrice.query.filter_by(symbol="ETH-USD")
        ...
        if eth_price_old == 0:
            print(
                "[ERROR] Previous ETH price is zero, cannot calculate percentage change"
            )
            return
    ...
    def refresh_NFT_base(user_id):
        ...
        existing_nfts = {
            nft.image_path: nft for nft in NFT.query.all()
        }  # Un dictionnaire pour une recherche rapide
        ...
        return {
            "status": "success",
            "message": f"NFT base refreshed successfully - {n_NFT_refreshed} NFTs added",
        }
    ...
    def get_NFT(nft_id, user_id):
        ...
        if NFTs:
            ...
            price_usd = round(
                CryptoDataManager().get_USD_from_crypto("ETH-USD", NFTs.price), 2
            )
            nft_data = {
                "id": NFTs.id,
                "name": NFTs.name,
            ...
            NFTs_list.append(
                {
                    ...
                    "number_of_likes": (
                        number_of_likes[nft_item.id]
                        if nft_item.id in number_of_likes
                        else 0
                    ),
                ...
                NFTs_list.append(
                    {
                        ...
                        "owner_id": nft_item.owner_id,
                        "liked": (
                            "" if nft_item.id in [n.nft_id for n in nft_ids] else "-o"
                        ),
                        "number_of_likes": (
                            number_of_likes[nft_item.id]
                            if nft_item.id in number_of_likes
    ...
    def like_NFT(user_id, nft_id):
        ...
        if (
            UserLikedNFT.query.filter_by(user_id=user_id, nft_id=nft_id).first()
            is not None
        ):
        ...
        if nft.is_for_sale:
            if eth_amount["tokens"] < nft_price:
                ...
                user_nft = UserNFT(
                    user_id=user_id,
                    nft_id=nft_id,
                    purchase_price_usd=CryptoDataManager().get_USD_from_crypto(
                        "ETH-USD", nft_price
                    ),
                    purchase_price_crypto=nft_price,
                    purchase_crypto_symbol="ETH",
                )
                ...
                history_nft = NFTPriceOwnerHistory(
                    nft_id=nft_id,
                    price=nft_price,
                    owner_id=user_id,
                ...
                # Update quests stats
                user_quest_stats = UserQuestsStats.query.filter_by(
                    user_id=user_id
                ).first()
                if user_quest_stats:
            ...
            return {
                "status": "error",
                "message": "The NFT is not for sale",
                "refresh": True,
            }
    ...
    def owned_status(user_id, nft_id):
        ...
        if nft.owner_id == user_id:
            return {
                ...
                "set_profile_text": (
                    ...
                    if nft_used_for_profile_picture
        ...
        if nft.owner_id is None:
            ...
            return {
                "message": f"Owned by <a href='{url}'>@{owner.username}</a>",
                "owned": False,
    ...
    def set_as_profile_picture(user_id, nft_id):
        ...
        if nft.owner_id != user_id:
            ...
            if user.profile_img_path == nft.image_path:
                ...
                return {
                    "status": "success",
                    "message": "The NFT is not your profile picture anymore",
                ...
                return {
                    "status": "success",
                    "message": "The NFT is now your profile picture",
    ...
    def place_bid(self, user_id, nft_id, amount):
        ...
        if nft.owner_id is None:
            return {
                "status": "error",
                "message": "The NFT is not for sale, just buy it",
            }
        ...
        for bid in nft_bids:
            if bid["user_id"] == user_id and bid["bid_price_crypto"] >= amount:
                return {
                    "status": "error",
                    "message": "You already placed a bid with a higher amount or same amount",
                }
    ...
    def delete_bid(bid_id, current_user_id):
        ...
        if bid.user_id != current_user_id and nft.owner_id != current_user_id:
            return {
                "status": "error",
                "message": "You are not the owner of the bid or the NFT",
            }
        # Return the amount of the bid to the user
        wallet = wallet_manager()
        wallet.receive_crypto(
            User.query.filter_by(id=bid.user_id).first(),
            "ETH-USD",
            bid.bid_price_crypto,
        )
    ...
    def accept_bid(bid_id, current_user_id):
        ...
        user_nft = UserNFT(
            ...
            nft_id=bid.nft_id,
            purchase_price_usd=CryptoDataManager().get_USD_from_crypto(
                "ETH-USD", bid.bid_price_crypto
            ),
            purchase_price_crypto=bid.bid_price_crypto,
            purchase_crypto_symbol="ETH",
        )
        db.session.add(user_nft)
