...
class CryptoWallet(db.Model):
    ...
    )  # Symbole de la cryptomonnaie, ex. BTC-USD
...
class GameWallet(db.Model):
    ...
    mini_wallet = db.Column(
        db.Float(200), nullable=False
    )  # Amount of money available in the game (refill every day)
    bank_wallet = db.Column(
        db.Float(10000), nullable=False
    )  # Amount of money available in the bank (refill every
    # week)
    mini_wallet_last_update = db.Column(
        db.DateTime, default=datetime.utcnow
    )  # Last update of the mini wallet
    bank_wallet_last_update = db.Column(
        db.DateTime, default=datetime.utcnow
    )  # Last update of the bank wallet
...
class CryptoWalletDailySnapshot(db.Model):
    ...
    )  # USD value of the wallet at this date
...
class CryptoPrice(db.Model):
    ...
    symbol = db.Column(
        db.String(10), nullable=False
    )  # Symbole de la cryptomonnaie, par ex. BTC
...
class UserServer(db.Model):
    ...
    server = db.relationship(
        "MiningServer", backref=db.backref("server_users", lazy=True)
    )
...
class NFT(db.Model):
    ...
    price = db.Column(
        db.Float, nullable=False
    )  # Price of the NFT at this moment in ETH
    image_path = db.Column(
        db.String(1000), nullable=False
    )  # Path to the image of the NFT
    ...
    price_change_24h = db.Column(
        db.Float, default=0
    )  # Pourcentage change of the price in the last 24h in USD
...
class NFTPriceOwnerHistory(db.Model):
    ...
    nft = db.relationship(
        "NFT", backref=db.backref("nft_price_owner_history", lazy=True)
    )
    owner = db.relationship(
        "User", backref=db.backref("user_nft_price_owner_history", lazy=True)
    )
...
class UserQuestsStats(db.Model):
    """
    Table to record the stats of the quests of the users
    - Unique number of NFTs bought through time (even if sold)
    - Unique number of NFTs sold through time
    - Number of bids made through time
    - Number of servers bought through time
    """

    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)  # User id
    nfts_bought = db.Column(
        db.Integer, nullable=False, default=0
    )  # Number of unique NFTs bought
    nfts_sold = db.Column(
        db.Integer, nullable=False, default=0
    )  # Number of unique NFTs sold
    bids_made = db.Column(db.Integer, nullable=False, default=0)  # Number of bids made
    servers_bought = db.Column(
        db.Integer, nullable=False, default=0
...
class UserQuestRewards(db.Model):
    ...
    )  # 'nfts_bought', 'nfts_sold', 'bids_made', 'servers_bought'
    step = db.Column(db.Integer, nullable=False)  # Step of the quest
    reward_claimed = db.Column(
...
class Position(db.Model):
    ...
    token_entry_price = db.Column(
        db.Float, nullable=False
    )  # price for 1 token at the entry
